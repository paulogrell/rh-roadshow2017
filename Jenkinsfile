...
podTemplate(
  ...
  node('myJenkinsMavenSlave') {
    ...
    stage('Publicando no Nexus') {
      ...
      sh "${mvnCmd} deploy -DskipTests=true -DaltDeploymentRepository=nexus::default::http://nexus.rhbrlab-cicd.svc.cluster.local:8081/repository/maven-releases"
    }
    ...
    stage('Build Imagem Openshift') {
      ...
      sh "oc policy add-role-to-user edit system:serviceaccount:demo-pipeline:jenkins -n rhforum-app-dev"
    ...
    stage('Preparando deploy em Producao') {
      ...
      sh "oc policy add-role-to-user edit system:serviceaccount:demo-pipeline:jenkins -n rhforum-app-prod"
    ...
    stage('Deploy em Producao') {
      ...
      openshiftDeploy depCfg: dest, namespace: 'rhforum-app-prod', verbose: 'true', waitTime: '', waitUnit: 'sec'
      openshiftVerifyDeployment depCfg: dest, namespace: 'rhforum-app-prod', replicaCount: '1', verbose: 'true', verifyReplicaCount: 'false', waitTime: '180000', waitUnit: 'sec'
